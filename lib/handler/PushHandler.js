...
function excuteUploadTasks(item) {
  workshop(function () {
    ...
    var spinner = ora('[PUSH] ' + 'Start: ' + startTime + ' | ' + item.path + ' | ' + (item.meta.size / (1024 * 1024)).toFixed(2) + 'Mb').start();
    return new Promise(function (resolve, reject) {
      qiniuWrapper.upload(path.join(item.query.dir, item.file), item.path)
        .then(function (ret) {
          ...
          spinner.succeed('[PUSH] ' + 'Start: ' + startTime + ' | ' + 'End: ' + moment().format('lll') + ' | ' + item.path
            + ' | ' + +(item.meta.size / (1024 * 1024)).toFixed(2) + 'Mb');
          return resolve(item);
        })
        .catch(function (err) {
          spinner.fail('[PUSH] ' + 'Start: ' + startTime + ' | ' + 'End: ' + moment().format('lll') + ' | ' + item.path
            + ' | ' + (item.meta.size / (1024 * 1024)).toFixed(2) + 'Mb');
          return reject(err);
        });
    });
